...
import mergeSort from '../../services/protocols/order-processor-protocols/merge-sort.js';
                      ...
                      if (subItem.id !== item.id) { //verifica se o subItem a ser combinado não é o mesmo itemPrincipal
                          ...
                          for (let i = 0; i < combinations[item.id].combinations.length; i++) { //percorre a lista de combinações do itemPrincipal
                              if (Object.values(combinations[item.id].combinations[i])[0] === subItem.id) { //verifica se existe a combinação do ItemPrincipal com o subItem
                                  ...
                                  addTopCombination({ [item.id]: { [newApparitionsValue]: subItem.id } }); // adiciona a combinação nos tops, somando quando ela já existir ou verificando se a nova soma é superior
                          ...
                          if (!isInserted) {
                              ...
                              addTopCombination({ [item.id]: { ["1"]: subItem.id } }); //tenta adicionar no array de melhoes combinacoes
                          }
                      }
                  }
              }
